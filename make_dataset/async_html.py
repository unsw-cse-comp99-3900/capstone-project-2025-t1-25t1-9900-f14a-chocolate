"""
异步加载 HTML 网页内容，解析网页中的文本，并封装为 Document 对象，以便用于进一步处理（例如文本分析或自然语言处理任务）。
"""
import asyncio
import logging
import warnings
...
class AsyncHtmlLoader(BaseLoader):
    ...
    def __init__(
        ...
        max_wait_time: int = 5,  # 新增属性，默认最大等待时长为 5 秒
    ):
        ...
        self.max_wait_time = max_wait_time  # 初始化 max_wait_time
    ...
    async def _fetch_with_rate_limit(
        ...
        async with semaphore:
            try:
                return await asyncio.wait_for(self._fetch(url, retries=1, cooldown=0, backoff=0), timeout=self.max_wait_time)
            except asyncio.TimeoutError:
                logger.warning(f"Fetching {url} timed out.")
    ...
    def load(self) -> List[Document]:
        ...
        for i, text in enumerate(cast(List[str], results)):
            if text == "Not available":
                logger.warning(f"Content from {self.web_paths[i]} was not available.")
                docs.append(Document(page_content='Not available', metadata={}))
